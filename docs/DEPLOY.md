...
3. **Settings → Secrets and variables → Actions**
   - Secret `QUEUE_DISPATCH_TOKEN` — Classic PAT, `repo` 권한
   - Secret `GH_PAT` (권장) — 워크플로가 리포트 파일 커밋할 때 사용
4. **Settings → General → Features** — **Issues** 켜기 (dispatch 토큰 없을 때 fallback)
5. Actions 탭에서 **Deploy SiteScope to GitHub Pages** 워크플로 성공 확인
